// stress2.t -- Stress tests for the garbage collector.
//
...
fun main(args: list of string): int
  ...
  x := 10000;
  while x > 0 do
    ls := string 256 of '!' + fun (x: string): string return x + ""; end("fooey") :: ls;
    x := x - 1;
  end;
  x := 80;
  while x > 0 do
  ...
  while ls <> null do
    ...
    if sizeof str <> 256 + 5 then
      return 1;
    end;
    var idx: int := 0;
    while idx < sizeof str - 5 do
      if str[idx] <> '!' then
	return 1;
      end;
      idx := idx + 1;
